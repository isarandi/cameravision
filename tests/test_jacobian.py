...
class TestJacobianNumerical:
    ...
    def test_jacobian_determinant_matches_numerical(self, d):
        ...
        analytical_det = cameravision.validity.jacobian_det_polar(r, t, d)
        ...
        for i, p in enumerate(points):
            ...
            p_xp = cameravision.distortion.distort_points(
                p.reshape(1, 2) + [[eps, 0]], d, check_validity=False
            )[0]
            p_xm = cameravision.distortion.distort_points(
                p.reshape(1, 2) - [[eps, 0]], d, check_validity=False
            )[0]
            p_yp = cameravision.distortion.distort_points(
                p.reshape(1, 2) + [[0, eps]], d, check_validity=False
            )[0]
            p_ym = cameravision.distortion.distort_points(
                p.reshape(1, 2) - [[0, eps]], d, check_validity=False
            )[0]
    ...
    def test_jacobian_symmetry(self):
        ...
        det = cameravision.validity.jacobian_det_polar(r, angles, d)
